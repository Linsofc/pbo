...
export default function Navbar({ username, onUpdateClick }: NavbarProps) {
    ...
    const fetchBalance = async () => {
        try {
            const res = await fetch("/api/digi/balance", { 
                method: "POST",
    ...
    const handleUpdateWrapper = async () => {
        ...
        try {
            // Mengirim log ke backend
            await sendLogToBackend("system", `User '${username}' memulai sinkronisasi data.`);
            
            // Memanggil fungsi update yang dikirim via props
            // Pastikan fungsi onUpdateClick di parent (page.tsx) memanggil API pricelist 
            // dengan menyertakan body: JSON.stringify({ username })
            const success = await onUpdateClick();
        ...
        <nav className="bg-[#121212] border-b border-gray-800 sticky top-0 z-50 font-sans">
            <div className="max-w-7xl mx-auto px-4 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    ...
                    {/* CENTER: NAV (Desktop) */}
                    <div className="hidden md:flex items-center gap-8 relative h-full" ref={navRef}>
                        {navItems.map((item, index) => {
                            ...
                            return (
                                <Link
                                    ...
                                    onMouseEnter={() => updateIndicator(tabsRef.current[index]!)}
                                    onMouseLeave={() => {
                                        const activeIndex = navItems.findIndex(i => i.path === pathname);
                                        if (activeIndex !== -1) updateIndicator(tabsRef.current[activeIndex]!);
                                    }}
                                    className={`relative z-10 h-full flex items-center px-2 text-sm font-bold tracking-wide transition-colors duration-300 ${isActive ? 'text-white' : 'text-gray-400 hover:text-white'}`}
                                >
                                    {item.name}
                                </Link>
                            );
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative" ref={dropdownRef}>
                            ...
                            >
                                <div className="hidden lg:flex flex-col items-end">
                                    <span className="text-xs font-bold text-white group-hover:text-blue-400 transition">{username || "Guest"}</span>
                                    <span className="text-[10px] text-blue-500 font-mono">{formatRupiah(saldo)}</span>
                                </div>
                                <div className="w-9 h-9 rounded-full bg-gray-800 p-[2px] ring-2 ring-transparent group-hover:ring-blue-500/50 transition">
                                    <img
                                        src={`https://ui-avatars.com/api/?name=${username || 'G'}&background=2563EB&color=fff&bold=true`}
                                        alt="User"
                            ...
                            {isProfileOpen && (
                                <div className="absolute right-0 mt-3 w-64 bg-[#1a1a1c] border border-gray-800 rounded-xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                        ...
                        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                {isMobileMenuOpen ? <path d="M6 18L18 6M6 6l12 12" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} /> : <path d="M4 6h16M4 12h16M4 18h16" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />}
                            </svg>
                        </button>
                    </div>
                </div>
            ...
            {/* MOBILE MENU */}
            {isMobileMenuOpen && (
                <div className="md:hidden bg-[#121212] border-t border-gray-800 absolute w-full left-0 shadow-2xl z-40 animate-in slide-in-from-top-5">
                    <div className="p-4 space-y-2">
                        {navItems.map((item) => (
                            <Link
                                ...
                                className={`block px-4 py-3 rounded-lg font-medium transition-colors ${pathname === item.path ? 'bg-blue-600/10 text-blue-400 border border-blue-600/20' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}
                            >
                                {item.name}
                            </Link>
                        ))}
