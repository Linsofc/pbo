...
export default function Home() {
  ...
  const processProducts = (rawData: Product[]) => {
    ...
    // Urutkan kategori: Games duluan
    const uniqueCats = Array.from(new Set(activeProducts.map((p) => p.category))).sort();
  ...
  const handleSyncData = async () => {
    try {
        ...
        if (result.success && Array.isArray(result.data) && result.data.length > 0) {
            ...
            showNotification(`Sukses! ${result.data.length} produk diperbarui.`, 'success');
            return true; 
        } else {
  ...
  const handleCheckId = async () => {
    ...
    if (selectedBrand === 'MOBILE LEGENDS' && !serverId) return showNotification('Masukkan Server ID!', 'info');
  ...
  const handleConfirmTransaction = async (e?: React.FormEvent) => {
    ...
    // 1. Cek PIN
    if (pin !== '123456') {
        showNotification('PIN Salah! Transaksi Dibatalkan.', 'error');
        setPin(''); // Reset input agar user bisa coba lagi
        return;
    }

    // 2. Jika PIN Benar, Lanjut Proses
    setIsPinModalOpen(false); // Tutup modal
    ...
    try {
        ...
        if (result.success) {
            ...
            const isPending = msg.includes('pending') || msg.includes('proses');
  ...
  const canCheckId = isGameCategory && (selectedBrand === 'MOBILE LEGENDS' || selectedBrand === 'FREE FIRE' || selectedBrand === 'HONOR OF KINGS');

  const getInputLabel = () => {
    if (isGameCategory) return isMLBB ? "User ID" : "User ID / Player ID";
    if (selectedCategory === 'Pulsa' || selectedCategory === 'Data') return "Nomor HP (08xx)";
  ...
  const getBrandImage = (brand: string) => {
    ...
    if(lower.includes('arena breakout')) return '/games/arena-breakout.png';
    ...
    if(lower.includes('tri') || lower.includes('three')) return '/pulsa/tri.png';
    ...
    return `https://placehold.co/300x400/18181b/FFF?text=${brand.substring(0,3)}`;
  };
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                {allProducts.length > 0 ? (
                    categories.map((category) => {
                        const brands = getBrandsByCategory(category).filter(b => 
                        ...
                        return (
                            <div key={category} className="mb-12 animate-in slide-in-from-bottom-4">
                                <div className="flex items-center gap-3 mb-6 border-b border-gray-800 pb-4">
                                    <div className={`w-1.5 h-8 rounded-full shadow-[0_0_15px] ${
                                        category === 'Games' ? 'bg-blue-600 shadow-blue-600/50' :
                                        category === 'Pulsa' ? 'bg-green-600 shadow-green-600/50' :
                                        category === 'E-Money' ? 'bg-purple-600 shadow-purple-600/50' : 
                                        'bg-gray-600'
                                    }`}></div>
                                    <h2 className="text-2xl font-bold tracking-tight text-white">{category}</h2>
                                    <span className="text-xs font-mono text-gray-500 bg-gray-800 px-2 py-1 rounded ml-2">{brands.length}</span>
                                </div>
                                    ...
                                    {brands.map((brand, i) => (
                                        ...
                                        >
                                            ...
                                            <div className="absolute bottom-0 w-full p-3 text-center z-10">
                                                <p className="text-xs sm:text-sm font-bold text-gray-100 group-hover:text-blue-400 transition-colors line-clamp-1 shadow-black drop-shadow-md">{brand}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 border border-dashed border-gray-800 rounded-3xl bg-[#18181b]/50">
                        <p className="text-gray-500 text-lg mb-2">Belum ada data produk.</p>
                        <p className="text-gray-600 text-sm">Klik profil di pojok kanan atas, lalu pilih "Sinkronisasi Data".</p>
                    </div>
                )}
            </div>
        ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in slide-in-from-bottom-8 duration-500 relative">
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-[#18181b]/80 backdrop-blur-xl p-6 rounded-3xl border border-white/5 sticky top-24 shadow-xl">
                        <div className="flex gap-5 items-start">
                            ...
                            />
                            <div>
                                <h1 className="text-xl font-bold leading-tight">{selectedBrand}</h1>
                                <p className="text-blue-400 text-xs mt-1 font-medium bg-blue-500/10 inline-block px-2 py-1 rounded border border-blue-500/20">{selectedCategory}</p>
                            </div>
                        </div>
                        ...
                        <button onClick={() => setSelectedBrand(null)} className="mt-6 w-full py-3 rounded-xl border border-gray-700 hover:bg-gray-800 transition text-sm font-medium">
                            ← Kembali ke Beranda
                        </button>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="bg-[#18181b]/80 backdrop-blur-xl p-6 rounded-3xl border border-white/5 shadow-lg relative overflow-hidden">
                        <div className="absolute top-0 left-0 bg-blue-600 px-4 py-2 rounded-br-2xl text-xs font-bold shadow-lg shadow-blue-600/20 z-10">1. Masukkan Tujuan</div>
                        
                        <div className="mt-6 grid grid-cols-2 gap-4">
                            <div className={`space-y-2 ${isMLBB ? '' : 'col-span-2'}`}>
                                <label className="text-xs text-gray-400 ml-1">{getInputLabel()}</label>
                                <input 
                                    type={isGameCategory ? 'text' : 'number'}
                        ...
                        {canCheckId && (
                            <div className="mt-4 flex items-center justify-between bg-[#0a0a0b] p-3 rounded-xl border border-gray-800">
                                <p className="text-xs text-gray-500">Cek nama akun sebelum membeli.</p>
                                <div className="flex items-center gap-3">
                                    {nickname && <span className="text-green-400 text-xs font-bold bg-green-500/10 px-3 py-1.5 rounded-lg border border-green-500/20">{nickname}</span>}
                                    <button onClick={handleCheckId} disabled={isCheckingId} className="bg-blue-600 hover:bg-blue-500 text-white text-xs px-4 py-2 rounded-lg font-bold transition shadow-lg shadow-blue-600/20">{isCheckingId ? '...' : 'Check'}</button>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-[#18181b]/80 backdrop-blur-xl p-6 rounded-3xl border border-white/5 shadow-lg relative overflow-hidden">
                        <div className="absolute top-0 left-0 bg-blue-600 px-4 py-2 rounded-br-2xl text-xs font-bold shadow-lg shadow-blue-600/20 z-10">2. Pilih Produk</div>
                            ...
                            {brandProducts.map((p) => (
                                ...
                                >
                                    <div className="relative z-10">
                                        <p className="text-xs text-gray-500 mb-1 line-clamp-2">{p.product_name.replace(selectedBrand, '').trim()}</p>
                                        <div className="mt-2 flex items-center justify-between pt-2 border-t border-gray-800">
                                            <p className="text-sm font-bold text-blue-400">Rp {p.price.toLocaleString('id-ID')}</p>
                                            {selectedSku === p.buyer_sku_code && (
                                    ...
                                    {selectedSku === p.buyer_sku_code && <div className="absolute inset-0 bg-blue-600/5 pointer-events-none"></div>}
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <button 
                        onClick={openPinModal} // Ganti handleBuy dengan openPinModal
            ...
            <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
                ...
                <div className="bg-[#18181b] border border-gray-700 rounded-2xl p-8 w-full max-w-sm relative z-10 shadow-2xl animate-in fade-in zoom-in duration-300 transform scale-100">
                    <div className="text-center mb-6">
                        ...
                        <h3 className="text-xl font-bold text-white">Konfirmasi Transaksi</h3>
                        <p className="text-gray-400 text-sm mt-1">Masukkan PIN keamanan Anda untuk melanjutkan pembelian.</p>
                    </div>
