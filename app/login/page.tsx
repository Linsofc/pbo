...
// import { sendLogToBackend } from '../../utils/logger'; // Gunakan ini jika error path alias
...
export default function LoginPage() {
  ...
  // State untuk mencegah flash content (kedip) saat pengecekan
  const [isCheckingAuth, setIsCheckingAuth] = useState(true);
  
  const router = useRouter();

  // 1. PROTEKSI: Jika sudah login, lempar ke Home ('/')
  useEffect(() => {
    const isLoggedIn = localStorage.getItem('isLoggedIn');
    if (isLoggedIn) {
      router.replace('/'); // Gunakan replace agar tidak bisa di-back
    } else {
      setIsCheckingAuth(false); // Buka akses halaman login
    }
  }, [router]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        await sendLogToBackend('auth', `User '${username}' berhasil login.`);
  ...
  // 2. TAMPILAN LOADING SEMENTARA (Agar form tidak muncul sekilas sebelum redirect)
  if (isCheckingAuth) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0b] relative overflow-hidden">
      ...
      <div className="w-full max-w-sm p-8 bg-[#18181b]/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-800 z-10 animate-in fade-in zoom-in duration-300">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-gray-400 text-sm">Masuk ke Linsofc Store</p>
        </div>
